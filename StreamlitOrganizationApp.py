...
with selectGraph:
    ...
    xx=list(df)
    st.header("Input your filters:")
    dimension = st.selectbox(" Graph Dimensions:", ("1D",'2D','3D'))
    gtype = st.selectbox("Select the Graph:", ("Beeswarm(1D)", "Scatter Plot (2D)", "Bar Graph (2D)", "Scatter Plot (3D)"))
    xvalue = st.selectbox("Select the X Value", (list(df)))
    yvalue = st.selectbox("Select Y Value", (list(df)))
    zvalue = st.selectbox("Select the Z Value", (list(df)))
    # create groups
