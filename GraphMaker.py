...
class graphtype():
    def __init__(self, dimension,gtype,yvalue,xvalue=None,zvalue=None):
...
def ChooseGraph(state,df,graphobj,labels): #iterate through groups in later iteration
    looper=len(graphobj)
    Particulars=[]
    uup=[]
    uupcount=0
    ...
    uwu=pd.concat(uup)
            ...
            #print(vils)
    if len(vils)==1:
        Particulars.append(vils)
    if graf=="Beeswarm(1D)" and len(vils)!=1:
        fig = px.strip(uwu, y=yg, hover_name="Institution", color="Group Labels")
        fig.show()
    ...
    if graf=="Scatter Plot (2D)":
        fig=px.scatter (uwu, x=xg,y=yg, hover_name="Institution", color="Group Labels")
        st.plotly_chart(fig, theme="streamlit", use_container_width=True)
         
    if graf=="Scatter Plot (3D)":
        fig = px.scatter_3d(uwu,x=xg,y=yg,z=zg, hover_name="Institution",  color="Group Labels")
        st.plotly_chart(fig, theme="streamlit", use_container_width=True)
